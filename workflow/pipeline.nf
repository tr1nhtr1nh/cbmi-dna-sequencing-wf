    ...
    */
    ...
    for i in "\${!databases[@]}"; do
        ...
        for fastq_file in ${fastq}/*.fastq; do
            ...
            blastn -db ${workflow.projectDir}/refseq/\${databases[i]}/\${file_names[i]} -outfmt "7 qseqid sseqid pident length evalue bitscore" -num_threads ${task.cpus} -query acc.fasta -out result.txt --keep-files
            rm acc.fasta
